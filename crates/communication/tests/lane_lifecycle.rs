//! Conformance tests for the two lane-lifecycle additions:
//!
...
//!    lets address-table collect an actor whose supervisor still holds a
...
use communication::{Config, Received, TrySendError, UserClosed, channel};
use std::sync::Arc;
...
// the shared suite in communication-testkit runs the same shape against both
// implementations.
