...
//! suite and allocation guard), so this file only measures
//! speed. It touches `communication` through its public API exclusively — a novel
...
/// The address-table wiring keeps this exact ratio — one anchor per actor.
...
async fn anchor_contention_ops_per_sec() -> f64 {
    ...
    // The counting sender stays ALIVE during contention (the address-table
